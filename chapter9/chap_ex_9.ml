(* 問題9.1 春、夏、秋、冬の4の文字列からなるリストを作れ *)

"春" :: "夏" :: "秋" :: "冬" :: []


(* 問題9.2 各人の名前,身長(m),体重(kg),誕生日(月と日)と血液型の情報を格納する長さ3のリストを作れ *)

"各人の名前" :: ("身長(m)" :: "体重(kg)" :: ("誕生日(月)" :: "誕生日(日)" :: []) :: []) :: "血液型" :: []

(* 問題9.3 春、夏、秋、冬の4の文字列からなるリストを要素を書き並べる方法で作れ *)

["春"; "夏"; "秋"; "冬" ] ;;

(* 問題9.4 整数のリストを受け取ったら,そのリストの長さを返す関数lengthをデザインレシピに従って作れ. *)
...
(* 問題9.5 整数のリストを受け取ったら,そのリストの偶数のみを返す関数evenをデザインレシピに従って作れ. *)
...
(* 問題9.6 文字リストを受け取ったら,その中の要素を前から順に全部くっつけた文字列を返す関数concatをデザインレシピに従って作れ. *)
...
(* 問題9.7 各人の名前,身長(m),体重(kg),誕生日(月と日)と血液型を格納するレコード型person_t を宣言し、データリスト宣言したら,血液型がAの人を返す関数count_ketueki_Aをデザインレシピにしたがって作れ *)
